...
<?php
	...
	foreach (array_keys($Channels) as $Channel) {
		...
		if (mysql_num_rows($d14InitResult) == 0) {
			...
			foreach($Channels[$Channel] as $Genre) {
				$d14EmptyVoteQuery .= "`artists`.`artist_genre` LIKE '%(".$Genre.")%' OR ";
			}
			$d14EmptyVoteQuery = substr($d14EmptyVoteQuery, 0, (strlen($d14EmptyVoteQuery) - 3));
			$d14EmptyVoteQuery .= ") ";
		...
		if (mysql_num_rows($d7InitResult) == 0) {
			...
			foreach($Channels[$Channel] as $Genre) {
				$d7EmptyVoteQuery .= "`artists`.`artist_genre` LIKE '%(".$Genre.")%' OR ";
			}
			$d7EmptyVoteQuery = substr($d7EmptyVoteQuery, 0, (strlen($d7EmptyVoteQuery) - 3));
			$d7EmptyVoteQuery .= ") ";
		...
		if ((mktime(0, 0, 0, date("m"), date("d"), date("Y")) - mktime(0, 0, 0, $LastVoteDate_Month, $LastVoteDate_Day, $LastVoteDate_Year)) >= VOTE_PERIOD) {
			...
			foreach($Channels[$Channel] as $Genre) {
				$d0VoteQuery .= "`artists`.`artist_genre` LIKE '%(".$Genre.")%' OR ";
			}
			$d0VoteQuery = substr($d0VoteQuery, 0, (strlen($d0VoteQuery) - 3));
			$d0VoteQuery .= ") AND (";
	...
	// RadioSTREAM - Full Shuffle
	$FS['Query'] = "SELECT `artist_id`, `album_id`, `track_id` FROM `tracks` WHERE (";
 	for ($TrackMins = 1; $TrackMins < $PlaylistMaxTrackTime; $TrackMins++) {
        	$FS['Query'] .= "`tracks`.`track_time` LIKE '";
        	if (strlen($TrackMins) == 1) {
        		$FS['Query'] .= "0";
        	}
        	$FS['Query'] .= $TrackMins.":%' OR ";
        }
        $FS['Query'] = substr($FS['Query'], 0, -4);
	$FS['Query'] .= ") AND (";
        $FS['Query'] .= "`track_genre` = '124' OR ";
        $FS['Query'] .= "`track_genre` = '75' OR ";
        $FS['Query'] .= "`track_genre` = '324' OR ";
        $FS['Query'] .= "`track_genre` = '1' OR ";
        $FS['Query'] .= "`track_genre` = '133' OR ";
        $FS['Query'] .= "`track_genre` = '155' OR ";
        $FS['Query'] .= "`track_genre` = '33' OR ";
        $FS['Query'] .= "`track_genre` = '3' OR ";
        $FS['Query'] .= "`track_genre` = '4' OR ";
        $FS['Query'] .= "`track_genre` = '172' OR ";
        $FS['Query'] .= "`track_genre` = '329' OR ";
        $FS['Query'] .= "`track_genre` = '200' OR ";
        $FS['Query'] .= "`track_genre` = '147' OR ";
        $FS['Query'] .= "`track_genre` = '179' OR ";
        $FS['Query'] .= "`track_genre` = '197' OR ";
        $FS['Query'] .= "`track_genre` = '210' OR ";
        $FS['Query'] .= "`track_genre` = '9' OR ";
        $FS['Query'] .= "`track_genre` = '15' OR ";
        $FS['Query'] .= "`track_genre` = '16' OR ";
        $FS['Query'] .= "`track_genre` = '158' OR ";
        $FS['Query'] .= "`track_genre` = '209' OR ";
        $FS['Query'] .= "`track_genre` = '14' OR ";
        $FS['Query'] .= "`track_genre` = '36'";
	$FS['Query'] .= ") ORDER BY RAND() LIMIT 2500";
	$FS['Result'] = mysql_query($FS['Query'], $MusicDB) or die("Error Full Shuffle: ".mysql_error());

	$FS['Playlist'] = PLAYLIST_PATH."RadioSTREAM-FullShuffle.pls";

	if(file_exists($FS['Playlist'])) {
		unlink(PLAYLIST_PATH."RadioSTREAM-FullShuffle.pls");
	}

	while ($FS['Row'] = mysql_fetch_array($FS['Result'])) {
		$fp = fopen($FS['Playlist'], "a+");
                fwrite($fp, MEDIA_PATH."artist/".$FS['Row']['artist_id']."/".$FS['Row']['album_id']."/".$FS['Row']['track_id'].".mp3\n");
                fclose($fp);
	}
	unset($FS);
